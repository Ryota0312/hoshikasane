 ...
 * 画像を比較明合成する
 */
pub fn lighten_composition(image1: &DynamicImage, image2: &DynamicImage) -> DynamicImage {
 ...
 * 画像を加算平均合成する
 */
pub fn average_composition(image1: &DynamicImage, image2: &DynamicImage) -> DynamicImage {
    let (width, height) = image1.dimensions();
    let mut image = RgbImage::new(width, height);
